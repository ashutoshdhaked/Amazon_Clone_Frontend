...
useEffect(()=>{
   ...
   if(socketmsg.reciverId === senderId){
    // whenever client is reading a new message from socket i want to change the to take the number of msg is increase and diplaying the current client chat
...
function checkUserINAllMsg(Msgdata){
   ...
   if(((allmsg[i].sender === Msgdata.sender) || (allmsg[i].sender === Msgdata.reciver)) && ((allmsg[i].reciver === Msgdata.sender) || (allmsg[i].reciver === Msgdata.reciver))){
    const customobj = {
      ...
      sendId :  Msgdata.msg.sendId
       ...
       getCurrentChatBtwSenderReciver(Msgdata.msg.recId,Msgdata.msg.sendId);  // if not a cuurent sender then i will not calling this  only increase the currosponding sender id message number 
       }
       else{
  ...
  if(checkvalue){
    ...
    console.log("allmsg first time : ",allmsg);
  }
}
...
async function getDataFromDb(id){
 ...
 if(response.status===200){
  ...
  if(!exist){
  allsender.push({sender : id, data : data , number : 1}); 
...
function sendMessage(){
  ...
  // we heve to change the senderId
  // we have to change the currentchat and find currentChat of particulare sender and reciver 
  // change the currentsender so change the particulare data 
   setreciverId(id); 
...
console.log("current sender : ",currentsender,reciverId);
